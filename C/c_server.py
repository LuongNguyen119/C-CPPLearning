#!/usr/bin/env python3
"""
C Compiler Server (port 8765) — Session + terminal-support API.
...
def _start_session(code):
    ...
    comp = subprocess.run(
        ["gcc", "-std=c11", "-Wall", "-Wextra", "-pedantic", str(src), "-o", str(exe)],
...
def main():
    ...
    print(f"C server running on http://{HOST}:{PORT}", flush=True)
    ...
    def reap_loop():
        while True:
            time.sleep(5)
            _reap()
    _thread.start_new_thread(reap_loop, ())
    try:
        srv.serve_forever()
