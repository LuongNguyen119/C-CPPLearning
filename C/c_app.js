(function () {
  ...
  function shuffleQuestion(question, seed) { const items = question.options.map((text, i) => ({ text, i })); const rand = mulberry32(hashCode(seed)); for (let i = items.length - 1; i > 0; i--) { const j = Math.floor(rand() * (i + 1)); [items[i], items[j]] = [items[j], items[i]]; } return { options: items.map((x) => x.text), answer: items.findIndex((x) => x.i === question.answer) }; }
  ...
  function renderQuiz(lesson) {
    ...
    $('quizBox').innerHTML = questions.map((q, qi) => { const v = shuffleQuestion(q, lesson.id + '|' + qi); return '<div class="quiz-question"><p><b>' + (qi + 1) + '.</b> ' + escape(q.question) + '</p><div class="quiz-options">' + v.options.map((option, oi) => '<button class="quiz-option" data-answer="' + oi + '" data-correct="' + v.answer + '">' + escape(option) + '</button>').join('') + '</div><p class="quiz-feedback" aria-live="polite"></p></div>'; }).join('');
  ...
  function renderExercise(id) {
    ...
    $('submitExercise').onclick = async () => { const results = []; for (const test of exercise.testCases) { try { const data = await callCodeApi({code: $('exerciseEditor').value, input: test.input || ''}); const actual = (data.output || '').trim(); results.push({ok: Boolean(data.ok) && actual === test.expected.trim(), actual, expected: test.expected}); } catch (error) { results.push({ok:false, actual:'Không kết nối backend', expected:test.expected}); } } const passed = results.filter((r) => r.ok).length; $('submissionResult').innerHTML = '<div class="result ' + (passed === results.length ? 'success' : 'failure') + '"><b>' + passed + '/' + results.length + ' tests passed · Score ' + Math.round(passed * 100 / results.length) + '/100</b>' + results.map((r, i) => '<p>' + (r.ok ? '✓' : '✕') + ' Test ' + (i + 1) + (r.ok ? '' : ' · Expected: ' + escape(r.expected) + ' · Got: ' + escape(r.actual)) + '</p>').join('') + '</div>'; state.exercises[exercise.id] = Object.assign({}, state.exercises[exercise.id], {completed: passed === results.length, bestScore: Math.max(passed * 100 / results.length, state.exercises[exercise.id]?.bestScore || 0), attempts: (state.exercises[exercise.id]?.attempts || 0) + 1}); save(); };
  }
  const API_BASE = /^(\[?127\.0\.0\.1\]?|\[?::1\]?|localhost):8765$/.test(window.location.host) ? '/api' : 'http://127.0.0.1:8765/api';
  ...
  async function callCodeApi(payload) {
    try {
      ...
      try { const response = await fetch(API_BASE + '/run', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload),signal:c.signal}); if (!response.ok) throw new Error('http ' + response.status); return await response.json(); }
      finally { clearTimeout(t); }
    } catch (error) {
      const wait = lastWandbox + 1200 - Date.now();
      if (wait > 0) await new Promise((r) => setTimeout(r, wait));
      lastWandbox = Date.now();
      const r = await fetch(WANDBOX, {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({compiler:'gcc-11.4.0-c',code:payload.code,stdin:payload.input || ''})});
  ...
  async function executeCode(code, input, outputNode) { outputNode.classList.remove('hidden'); outputNode.textContent = 'Đang biên dịch và chạy...'; try { const data = await callCodeApi({code,input}); outputNode.textContent = (data.phase === 'compile' ? 'Lỗi biên dịch:\n' : 'Kết quả chạy:\n') + (data.output || 'Không có output.'); } catch (error) { outputNode.textContent = 'Không chạy được code: không thấy compiler local (127.0.0.1:8765) và không gọi được Wandbox API (cần internet). Chạy python C/c_server.py rồi thử lại.'; } }
